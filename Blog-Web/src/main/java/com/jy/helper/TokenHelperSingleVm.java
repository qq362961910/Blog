...
@Component
public class TokenHelperSingleVm {

    private static final Logger logger = LogManager.getLogger(TokenHelperSingleVm.class);
    ...
    public TokenHelperSingleVm() {
        this(null);
    }

    public TokenHelperSingleVm(TokenLoader tokenLoader) {
        this(DEFAULT_TOKEN_EXPIRE_TIIME_IN_MILLS, DEFAULT_TOKEN_SERVICE_LIFE_TIMEOUT_IN_MILLS, tokenLoader);
    }

    public TokenHelperSingleVm(int tokenExpireTime, int serviceLifeTimeout, TokenLoader defaultTokenLoader) {
                ...
                while (!closed) {
                    try {
                        ...
                        for (Map.Entry<String, Long> entry : tokenInUseMapping.entrySet()) {
                            //判断是否使用超时
                            if (currentTimeMillis - TokenHelperSingleVm.this.serviceLifeTimeout > entry.getValue()) {
